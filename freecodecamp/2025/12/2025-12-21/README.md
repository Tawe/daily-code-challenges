...
My initial approach focused on mapping exact latitude values to daylight hours using an object. This worked for the examples provided in the table, but it didn’t fully account for the requirement that non-exact latitude values should use the closest entry. Once I revisited the problem statement, it became clear that this wasn’t a direct lookup problem—it was a comparison problem. The input latitude could fall between table entries, so I needed a way to determine which known latitude was nearest. Reframing the table as numeric data instead of key–value pairs made the solution clearer and more flexible. Instead of relying on exact matches, I could evaluate distance and choose the best approximation.
