...
This challenge was a good example of when regular expressions are actually the right tool for the job. The input format is very strict and well-defined, so instead of manually parsing the string, a single regex capture was enough to extract everything I needed. I focused on making sure the output HTML matched the required format exactly — especially the attribute order, spacing, and quotation marks. Small formatting differences would break the expected result even if the logic was otherwise correct. It was also interesting to think about how much the problem assumes valid input. Given that assumption, the solution can stay very compact and readable without extra defensive checks.
