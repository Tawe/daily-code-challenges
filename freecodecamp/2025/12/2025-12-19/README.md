...
My first instinct was to use a nested loop and check every possible pair of numbers. That approach is straightforward and easy to reason about, and it works for small inputs and simple examples. However, after thinking more carefully about the problem rules, I realized that each element can only be used once. My initial solution didn’t enforce that constraint and would overcount pairs when multiple combinations reused the same indices. That forced me to rethink the problem. Instead of looking at all possible pairs, I needed a way to remember which values had already been seen and whether they could form a valid pair with the current element—while also making sure each index was only used once. Reframing the problem as a single pass with a lookup structure made the solution both cleaner and more efficient.
