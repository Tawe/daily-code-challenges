...
- Sorting then slicing is a simple, readable way to remove extremes when the array is small. - Rest parameters are great for optional, variable-length inputs like penalties. - `reduce` can keep scoring logic concise, but it’s worth remembering `sort` mutates the original array.
