...
I solved this with a mapping approach:

- find the best score with `Math.max(...jumpScores)`
- map each score to a delay using `(bestScore - score) * 1.5`
- round up each delay with `Math.ceil(...)`

That directly matches the problem formula, and it keeps the code short and easy to read.
