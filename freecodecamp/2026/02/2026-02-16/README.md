...
This challenge is basically parse -> score -> rank -> format. I split each entry like `"FIN: 2-2-1-0"` into team name and record, then parse wins/overtime wins/overtime losses/losses as numbers. From there, scoring is a direct formula: - regulation wins: `wins * 3`
- overtime wins: `otWins * 2`
- overtime losses: `otLosses * 1`
- losses: `0` After computing points for each team, I sort descending by points and pick the top four seeds in order (`1st`, `2nd`, `3rd`, `4th`). Then I build the exact output sentence using the required pairings `1 vs 4` and `2 vs 3`.
