...
This one looked simple at first, but the key detail is that the minimum requirement applies to the **sled only**, while the maximum applies to **athletes + sled**. I handled it by: - using `athleteWeights.length` to determine which team rules apply
...
- returning `"Not Eligible"` when `totalWeight > maxTotalWeight` Otherwise the team is `"Eligible"`.
