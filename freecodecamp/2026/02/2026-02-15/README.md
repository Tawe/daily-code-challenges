...
This one is a straightforward formula + threshold classification problem. I first compute base steepness as `drop / distance`, then apply a multiplier based on `type` (`Downhill`, `Slalom`, `Giant Slalom`) to get adjusted steepness.

After that, I classify with ordered boundary checks:

- `<= 0.1` => `Green`
- `<= 0.25` => `Blue`
- otherwise => `Black`

Using the conditions in ascending order keeps the comparisons simple and avoids overlapping cases.
